...
struct work_load {
  ...
  work_load(const Range& range, const ParallelLoopBody& body, int nstripes) {
...
class ForThread {
 public:
  ForThread()
      : m_task_start(false), m_parent(0), m_state(eFTNotStarted), m_id(0) {}
...
const char ThreadManager::m_env_name[] = "CIL_FOR_THREADS_NUM";

#ifdef ANDROID
// many modern phones/tablets have 4-core CPUs. Let's use no more
...
void ForThread::execute() {
  unsigned int m_current_pos = CIL_XADD(&m_parent->m_task_position, 1);
  ...
  while (m_current_pos < load.m_nstripes) {
    ...
    load.m_body->operator()(Range(start, end));

    m_current_pos = CIL_XADD(&m_parent->m_task_position, 1);
  }
}
...
void ThreadManager::run(const Range& range,
                        const ParallelLoopBody& body,
                        double nstripes) {
  ...
  if ((getNumOfThreads() > 1) && !is_work_thread &&
    ...
    if (!res) {
      if (initPool()) {
        ...
        m_work_load.set(range, body, fast_ceil(nstripes));
...
void ThreadManager::notify_complete() {
  unsigned int comp = CIL_XADD(&m_num_of_completed_tasks, 1);
...
void parallel_for_pthreads(const Range& range,
                           const ParallelLoopBody& body,
                           double nstripes);
