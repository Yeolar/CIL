...
/* IMPORTANT: always use the same order of defines
   ...
   6. HAVE_CONCURRENCY - part of runtime, used automatically (Windows only -
                         MSVS 10, MSVS 11)
...
#if defined HAVE_TBB
#define CIL_PARALLEL_FRAMEWORK "tbb"
#elif defined HAVE_CSTRIPES
#define CIL_PARALLEL_FRAMEWORK "cstripes"
#elif defined HAVE_OPENMP
#define CIL_PARALLEL_FRAMEWORK "openmp"
#elif defined HAVE_GCD
#define CIL_PARALLEL_FRAMEWORK "gcd"
#elif defined WINRT
#define CIL_PARALLEL_FRAMEWORK "winrt-concurrency"
#elif defined HAVE_CONCURRENCY
#define CIL_PARALLEL_FRAMEWORK "ms-concurrency"
#elif defined HAVE_PTHREADS_PF
#define CIL_PARALLEL_FRAMEWORK "pthreads"
#endif

namespace cil {

#ifdef HAVE_PTHREADS_PF
void parallel_for_pthreads(const cil::Range& range,
                           const cil::ParallelLoopBody& body,
                           double nstripes);
size_t parallel_pthreads_get_threads_num();
void parallel_pthreads_set_threads_num(int num);
#endif

#ifdef CIL_PARALLEL_FRAMEWORK

class ParallelLoopBodyWrapper : public cil::ParallelLoopBody {
 public:
  ParallelLoopBodyWrapper(const cil::ParallelLoopBody& _body,
                          const cil::Range& _r,
                          double _nstripes) {
    body = &_body;
    wholeRange = _r;
    ...
    nstripes = fast_round(
        _nstripes <= 0 ? len : std::min(std::max(_nstripes, 1.), len));
  }
  void operator()(const cil::Range& sr) const {
    cil::Range r;
    r.start = (int)(wholeRange.start +
                    ((uint64_t)sr.start * (wholeRange.end - wholeRange.start) +
                     nstripes / 2) /
                        nstripes);
    r.end = sr.end >= nstripes
                ? wholeRange.end
                : (int)(wholeRange.start +
                        ((uint64_t)sr.end * (wholeRange.end - wholeRange.start) +
                         nstripes / 2) /
                            nstripes);
    (*body)(r);
  }
  cil::Range stripeRange() const {
    return cil::Range(0, nstripes);
  }

 protected:
  const cil::ParallelLoopBody* body;
  cil::Range wholeRange;
  int nstripes;
};
...
class ProxyLoopBody : public ParallelLoopBodyWrapper {
 public:
  ProxyLoopBody(const cil::ParallelLoopBody& _body,
                const cil::Range& _r,
                double _nstripes)
  ...
  void operator()(const tbb::blocked_range<int>& range) const {
    this->ParallelLoopBodyWrapper::operator()(
        cil::Range(range.begin(), range.end()));
  }
};
...
static void block_function(void* context, size_t index) {
  ...
  (*ptr_body)(cil::Range((int)index, (int)index + 1));
}
...
class ProxyLoopBody : public ParallelLoopBodyWrapper {
 public:
  ...
  void operator()(int i) const {
    this->ParallelLoopBodyWrapper::operator()(cil::Range(i, i + 1));
  }
};
#else
typedef ParallelLoopBodyWrapper ProxyLoopBody;
...
#endif  // CIL_PARALLEL_FRAMEWORK

/* ================================   parallel_for_
 * ================================ */

void parallel_for_(const cil::Range& range,
                   const cil::ParallelLoopBody& body,
                   double nstripes) {
  if (range.empty())
    return;
  ...
  if (numThreads != 0) {
    ...
    cil::Range stripeRange = pbody.stripeRange();
    ...
    tbb::parallel_for(
        tbb::blocked_range<int>(stripeRange.start, stripeRange.end), pbody);
...
#pragma omp parallel for schedule(dynamic)
    ...
    dispatch_apply_f(stripeRange.end - stripeRange.start, concurrent_queue,
    ...
    if (!pplScheduler ||
        pplScheduler->Id() == Concurrency::CurrentScheduler::Id()) {
      Concurrency::parallel_for(stripeRange.start, stripeRange.end, pbody);
    } else {
      pplScheduler->Attach();
      Concurrency::parallel_for(stripeRange.start, stripeRange.end, pbody);
    ...
    parallel_for_pthreads(pbody.stripeRange(), pbody,
                          pbody.stripeRange().size());
...
int getNumThreads(void) {
  ...
  return numThreads > 0 ? numThreads : cil::getNumberOfCPUs();
...
// there is only private dispatch_queue_set_width() and only for desktop
  ...
  if (threads <= 0) {
    ...
    // Concurrency always uses >=2 threads, so we just disable it if 1 thread is
    // requested
    numThreads = 0;
  } else if (pplScheduler == 0 ||
             1 + pplScheduler->GetNumberOfVirtualProcessors() !=
                 (unsigned int)threads) {
    pplScheduler = Concurrency::Scheduler::Create(Concurrency::SchedulerPolicy(
        2, Concurrency::MinConcurrency, threads - 1,
...
#elif defined HAVE_CONCURRENCY
  return std::max(
      0, (int)Concurrency::Context::VirtualProcessorId());  // zero for master
                                                            // thread, unique
                                                            // number for
