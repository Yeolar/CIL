...
/*
 ...
 If a temporary buffer is usually small (a few K's of memory), but its size
 depends on the parameters, it makes sense to create a small fixed-size array
 on stack and use it if it's large enough. If the required buffer size is
 larger than the fixed size, another buffer of sufficient size is allocated
 dynamically and released after the processing. Therefore, in typical cases,
 when the buffer size is small, there is no overhead associated with
 ...
 buffer elements and the number of stack-allocated elements.
 Here is how the class is used:
    ...
    buf.allocate(rows);   // if rows <= 1000, the pre-allocated buffer is used,
                          // otherwise the buffer of "rows" floats will be
                          // allocated dynamically and deallocated in
                          // cv::AutoBuffer destructor
*/
