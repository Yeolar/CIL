...
/** @brief Template function for accurate conversion from one primitive type to
 ...
 The functions saturate_cast resemble the standard C++ cast operations, such as
 static_cast\<T\>() and others. They perform an efficient and accurate
 conversion from one primitive type to another (see the introduction chapter).
 saturate in the name means that when the input value v is out of the range of
 the target type, the result is not formed just by taking low bits of the
 input, but instead the value is clipped. For example:
 @code
...
template <>
inline uchar saturate_cast<uchar>(char v) {
...
inline uchar saturate_cast<uchar>(int64_t v) {
  return (uchar)((uint64_t)v <= (uint64_t)UCHAR_MAX
                    ? v
                    : v > 0 ? UCHAR_MAX : 0);
}
template <>
...
template <>
inline char saturate_cast<char>(uchar v) {
  return (char)std::min((int)v, SCHAR_MAX);
}
template <>
inline char saturate_cast<char>(ushort v) {
  return (char)std::min((unsigned)v, (unsigned)SCHAR_MAX);
}
template <>
inline char saturate_cast<char>(int v) {
  return (char)((unsigned)(v - SCHAR_MIN) <= (unsigned)UCHAR_MAX
                    ? v
...
template <>
inline char saturate_cast<char>(unsigned v) {
  return (char)std::min(v, (unsigned)SCHAR_MAX);
}
template <>
inline char saturate_cast<char>(int64_t v) {
  return (char)((uint64_t)((int64_t)v - SCHAR_MIN) <= (uint64_t)UCHAR_MAX
                    ? v
                    : v > 0 ? SCHAR_MAX : SCHAR_MIN);
}
template <>
inline char saturate_cast<char>(uint64_t v) {
  return (char)std::min(v, (uint64_t)SCHAR_MAX);
}
template <>
...
template <>
inline ushort saturate_cast<ushort>(char v) {
...
inline ushort saturate_cast<ushort>(int v) {
  return (ushort)((unsigned)v <= (unsigned)USHRT_MAX
                      ? v
                      : v > 0 ? USHRT_MAX : 0);
}
template <>
...
inline ushort saturate_cast<ushort>(int64_t v) {
  return (ushort)((uint64_t)v <= (uint64_t)USHRT_MAX
                      ? v
                      : v > 0 ? USHRT_MAX : 0);
}
template <>
