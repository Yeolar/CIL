...
bool JpegDecoder::readHeader() {
  ...
  if (setjmp(state->jerr.setjmp_buffer) == 0) {
    ...
    if (!m_buf.empty()) {
      ...
      state->source.pub.bytes_in_buffer =
          m_buf.cols * m_buf.rows * m_buf.elemSize();
    } else {
    ...
    if (state->cinfo.src != 0) {
      ...
      m_type = state->cinfo.num_components > 1 ? CIL_8UC3 : CIL_8UC1;
      result = true;
    }
  }
      ...
      for (; m_height--; data += step) {
        ...
        if (color) {
          if (cinfo->out_color_components == 3)
            memcpy(data, buffer[0], 3 * m_width);
          else
            convert_CMYK2BGR_8u_C4C3R(buffer[0], 0, data, 0, Size(m_width, 1));
        } else {
          if (cinfo->out_color_components == 1)
            memcpy(data, buffer[0], m_width);
          else
            convert_CMYK2Gray_8u_C4C1R(buffer[0], 0, data, 0, Size(m_width, 1));
        }
      }
  ...
  if (setjmp(jerr.setjmp_buffer) == 0) {
    ...
    for (size_t i = 0; i < params.size(); i += 2) {
      if (params[i] == IMWRITE_JPEG_QUALITY) {
        quality = params[i + 1];
        quality = std::min(std::max(quality, 0), 100);
      }
      ...
      if (params[i] == IMWRITE_JPEG_LUMA_QUALITY) {
        if (params[i + 1] >= 0) {
          luma_quality = std::min(std::max(params[i + 1], 0), 100);
      ...
      if (params[i] == IMWRITE_JPEG_CHROMA_QUALITY) {
        if (params[i + 1] >= 0) {
          chroma_quality = std::min(std::max(params[i + 1], 0), 100);
        }
      }
      ...
      if (params[i] == IMWRITE_JPEG_RST_INTERVAL) {
        ...
        rst_interval =
            std::min(static_cast<long int>(std::max(rst_interval, 0)), 65535L);
      }
    }
    ...
    for (int y = 0; y < height; y++) {
      ...
      if (_channels == 3) {
        memcpy(buffer, data, _channels * width);
        ptr = buffer;
      } else if (_channels == 4) {
        convert_RGBA2RGB_8u_C4C3R(data, 0, buffer, 0, Size(width, 1), 2);
        ptr = buffer;
      }
