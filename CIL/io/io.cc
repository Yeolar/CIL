...
#include "CIL/error.h"
#include "CIL/file.h"
#include "CIL/io/base.h"
#include "CIL/io/exif.h"
#include "CIL/io/jpeg.h"
#include "CIL/io/options.h"
#include "CIL/io/png.h"
...
static void ApplyExifOrientation(const String& filename, Mat& img) {
  ...
  case IMAGE_ORIENTATION_TR:  // 0th row == visual top, 0th column == visual
    ...
    break;
  case IMAGE_ORIENTATION_BR:  // 0th row == visual bottom, 0th column == visual
                              // right-hand side
    flip(img, img, -1);       // flip both horizontally and vertically
    break;
  case IMAGE_ORIENTATION_BL:  // 0th row == visual bottom, 0th column == visual
                              // left-hand side
...
static void* imread_(const String& filename, int flags, Mat* mat = 0) {
  ...
  if ((flags & IMREAD_LOAD_GDAL) != IMREAD_LOAD_GDAL &&
    ...
    if ((flags & CIL_LOAD_IMAGE_ANYDEPTH) == 0)
      type = CIL_MAKETYPE(CIL_8U, CIL_MAT_CN(type));

    if ((flags & CIL_LOAD_IMAGE_COLOR) != 0 ||
        ((flags & CIL_LOAD_IMAGE_ANYCOLOR) != 0 && CIL_MAT_CN(type) > 1))
      type = CIL_MAKETYPE(CIL_MAT_DEPTH(type), 3);
    else
      type = CIL_MAKETYPE(CIL_MAT_DEPTH(type), 1);
  }
  ...
  {
    // TODO(saumitro)
    //        resize(*mat, *mat, Size(size.width / scale_denom, size.height /
    //        scale_denom));
  }
...
static bool imwrite_(const String& filename,
                     const Mat& image,
  ...
  CIL_ASSERT(image.channels() == 1 || image.channels() == 3 ||
               image.channels() == 4);

  ImageEncoder encoder = findEncoder(filename);
  if (!encoder)
    CIL_ERROR("could not find a writer for the specified extension");
  if (!encoder->isFormatSupported(image.depth())) {
    CIL_ASSERT(encoder->isFormatSupported(CIL_8U));
    image.convertTo(temp, CIL_8U);
    pimage = &temp;
  }
...
static void* imdecode_(const Mat& buf, int flags, Mat* mat = 0) {
  CIL_ASSERT(!buf.empty() && buf.isContinuous());
  Mat temp, *data = &temp;
  String filename;
  ...
  if (!decoder->readHeader()) {
    ...
    if (!filename.empty()) {
      if (remove(filename.c_str()) != 0) {
        CIL_ERROR("unable to remove temporary file");
      }
    }
    return 0;
  }
...
bool imencode(const String& ext,
  ...
  CIL_ASSERT(channels == 1 || channels == 3 || channels == 4);

  ImageEncoder encoder = findEncoder(ext);
  if (!encoder)
    CIL_ERROR("could not find encoder for the specified extension");
