...
ImageDecoder PngDecoder::newDecoder() const {
  return std::make_shared<PngDecoder>();
}
      ...
      if (setjmp(png_jmpbuf(png_ptr)) == 0) {
        ...
        if (!m_buf.empty() || m_f) {
          ...
          if (bit_depth <= 8 || bit_depth == 16) {
            ...
            if (bit_depth == 16)
              m_type = CIL_MAKETYPE(CIL_16U, CIL_MAT_CN(m_type));
            result = true;
          }
        }
      }
...
bool PngDecoder::readData(Mat& img) {
  ...
  if (m_png_ptr && m_info_ptr && m_end_info && m_width && m_height) {
    ...
    if (setjmp(png_jmpbuf(png_ptr)) == 0) {
      ...
      if (img.depth() == CIL_8U && m_bit_depth == 16)
...
bool PngEncoder::isFormatSupported(int depth) const {
  return depth == CIL_8U || depth == CIL_16U;
}
...
void PngEncoder::writeDataToBuf(void* _png_ptr, uchar* src, size_t size) {
  ...
  CIL_ASSERT(encoder && encoder->m_buf);
      ...
      if (setjmp(png_jmpbuf(png_ptr)) == 0) {
        ...
        for (size_t i = 0; i < params.size(); i += 2) {
          if (params[i] == IMWRITE_PNG_COMPRESSION) {
            ...
            compression_level =
                std::min(std::max(compression_level, 0), Z_BEST_COMPRESSION);
          }
          if (params[i] == IMWRITE_PNG_STRATEGY) {
            compression_strategy = params[i + 1];
            compression_strategy =
                std::min(std::max(compression_strategy, 0), Z_FIXED);
          }
