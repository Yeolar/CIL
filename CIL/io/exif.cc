 ...
 */
ExifReader::ExifReader(std::string filename)
    : m_filename(filename), m_format(NONE) {}
 ...
 * @return  true if parsing was successful and exif information exists in
 * JpegReader object
 ...
 *
 *  @return ExifEntru_t structure. Caller has to know what tag it calls in order
 * to extract proper field from the
 * structure ExifEntry_t
 *
 */
...
std::map<int, ExifEntry_t> ExifReader::getExif() {
  ...
  const size_t offsetToTiffHeader =
  ...
  while ((!feof(f)) && !exifFound && !stopSearch) {
    ...
    size_t exifSize;
    switch (marker) {
    // For all the markers just skip bytes in file pointed by followed two bytes
    // (field size)
    case SOF0:
    case SOF2:
    case DHT:
    ...
    case APP1:  // actual Exif Marker
      ...
      count = fread(&m_data[0], sizeof(unsigned char),
                    exifSize - offsetToTiffHeader, f);
      exifFound = true;
      break;

    default:  // No other markers are expected according to standard. May be a
              // signal of error
      stopSearch = true;
      break;
    }
...
/**
 * @brief Get the size of exif field (required to properly ready whole exif from
 * the file)
 *          This is internal function and is not exposed to client
 *
 ...
 *          This is internal function and is not exposed to client
 *
 *  @return The function doesn't return any value. In case of unsiccessful
 * parsing
...
/**
 * @brief Checking whether Tag Mark (0x002A) correspond to one contained in the
 * Jpeg file
 *          This is internal function and is not exposed to client
 *
...
/**
 * @brief The utility function for extracting actual offset exif IFD0 info is
 * started from
 *          This is internal function and is not exposed to client
 *
...
uint32_t ExifReader::getU32(const size_t offset) const {
  ...
  return (m_data[offset] << 24) + (m_data[offset + 1] << 16) +
         (m_data[offset + 2] << 8) + m_data[offset + 3];
}
 ...
 *
 * "rational" means a fractional value, it contains 2 signed/unsigned long
 * integer value,
 ...
 *
 */
std::vector<u_rational_t> ExifReader::getPrimaryChromaticies(
 ...
 *
 */
std::vector<u_rational_t> ExifReader::getYCbCrCoeffs(
    const size_t offset) const {
  std::vector<u_rational_t> result;
  uint32_t rationalOffset = getU32(offset + 8);
