...
void scalarToRawData(const Scalar& s, void* _buf, int type, int unroll_to) {
  int i, depth = CIL_MAT_DEPTH(type), cn = CIL_MAT_CN(type);
  CIL_ASSERT(cn <= 4);
  switch (depth) {
  case CIL_8U: {
  ...
  case CIL_8S: {
    char* buf = (char*)_buf;
    for (i = 0; i < cn; i++)
      buf[i] = saturate_cast<char>(s.val[i]);
    for (; i < unroll_to; i++)
      buf[i] = buf[i - cn];
  } break;
  case CIL_16U: {
...
template <typename T>
static void copyMask_(const uchar* _src,
                      size_t sstep,
                      const uchar* mask,
                      size_t mstep,
                      uchar* _dst,
...
template <>
void copyMask_<uchar>(const uchar* _src,
                      size_t sstep,
                      const uchar* mask,
                      size_t mstep,
                      uchar* _dst,
...
template <>
void copyMask_<ushort>(const uchar* _src,
                       size_t sstep,
                       const uchar* mask,
                       size_t mstep,
                       uchar* _dst,
  ...
  for (; size.height--; mask += mstep, _src += sstep, _dst += dstep) {
    ...
    for (; x <= size.width - 8; x += 8) {
      ...
      uint16x8_t v_mask_res =
          vreinterpretq_u16_u8(vcombine_u8(v_mask2.val[0], v_mask2.val[1]));
...
static void copyMaskGeneric(const uchar* _src,
                            size_t sstep,
                            const uchar* mask,
                            size_t mstep,
                            uchar* _dst,
...
#define DEF_COPY_MASK(suffix, type)                                         \
  static void copyMask##suffix(const uchar* src, size_t sstep,              \
                               const uchar* mask, size_t mstep, uchar* dst, \
...
void Mat::copyTo(OutputArray _dst) const {
  ...
  if (_dst.fixedType() && dtype != type()) {
    CIL_ASSERT(channels() == CIL_MAT_CN(dtype));
  ...
  if (dims <= 2) {
    ...
    if (rows > 0 && cols > 0) {
      // For some cases (with vector) dst.size != src.size, so force to
      // column-based form
...
void Mat::copyTo(OutputArray _dst, InputArray _mask) const {
  ...
  CIL_ASSERT(mask.depth() == CIL_8U && (mcn == 1 || mcn == cn));
...
Mat& Mat::setTo(InputArray _value, InputArray _mask) {
  ...
  CIL_ASSERT(checkScalar(value, type(), _value.kind(), _InputArray::MAT));
  CIL_ASSERT(mask.empty() || (mask.type() == CIL_8U && size == mask.size));
  ...
  uchar* ptrs[2] = {0, 0};
  NAryMatIterator it(arrays, ptrs);
  int totalsz = (int)it.size,
      blockSize0 = std::min(totalsz, (int)((BLOCK_SIZE + esz - 1) / esz));
