#include "CIL/mat/convert.h"

#include <cfloat>
#include <cstring>

#include "CIL/hardware.h"
#include "CIL/numeric.h"
#include "CIL/util.h"
...
struct cvtScaleAbs_SIMD<uchar, uchar, float> {
  int operator()(
      const uchar* src, uchar* dst, int width, float scale, float shift) const {
    int x = 0;
    ...
    if (USE_SSE2) {
      ...
      for (; x <= width - 16; x += 16) {
        ...
        __m128i v_src12 = _mm_unpacklo_epi8(v_src, v_zero_i),
                v_src_34 = _mm_unpackhi_epi8(v_src, v_zero_i);
        __m128 v_dst1 = _mm_add_ps(
            _mm_mul_ps(_mm_cvtepi32_ps(_mm_unpacklo_epi16(v_src12, v_zero_i)),
                       v_scale),
            v_shift);
        v_dst1 = _mm_max_ps(_mm_sub_ps(v_zero_f, v_dst1), v_dst1);
        __m128 v_dst2 = _mm_add_ps(
            _mm_mul_ps(_mm_cvtepi32_ps(_mm_unpackhi_epi16(v_src12, v_zero_i)),
                       v_scale),
            v_shift);
        v_dst2 = _mm_max_ps(_mm_sub_ps(v_zero_f, v_dst2), v_dst2);
        __m128 v_dst3 = _mm_add_ps(
            _mm_mul_ps(_mm_cvtepi32_ps(_mm_unpacklo_epi16(v_src_34, v_zero_i)),
                       v_scale),
            v_shift);
        v_dst3 = _mm_max_ps(_mm_sub_ps(v_zero_f, v_dst3), v_dst3);
        __m128 v_dst4 = _mm_add_ps(
            _mm_mul_ps(_mm_cvtepi32_ps(_mm_unpackhi_epi16(v_src_34, v_zero_i)),
                       v_scale),
            v_shift);
        v_dst4 = _mm_max_ps(_mm_sub_ps(v_zero_f, v_dst4), v_dst4);

        __m128i v_dst_i = _mm_packus_epi16(
            _mm_packs_epi32(_mm_cvtps_epi32(v_dst1), _mm_cvtps_epi32(v_dst2)),
...
template <>
struct cvtScaleAbs_SIMD<char, uchar, float> {
  int operator()(
      const char* src, uchar* dst, int width, float scale, float shift) const {
    int x = 0;
    ...
    if (USE_SSE2) {
      ...
      for (; x <= width - 16; x += 16) {
        __m128i v_src = _mm_loadu_si128((const __m128i*)(src + x));
        __m128i v_src_12 =
                    _mm_srai_epi16(_mm_unpacklo_epi8(v_zero_i, v_src), 8),
                v_src_34 =
                    _mm_srai_epi16(_mm_unpackhi_epi8(v_zero_i, v_src), 8);
        __m128 v_dst1 = _mm_add_ps(
            _mm_mul_ps(_mm_cvtepi32_ps(_mm_srai_epi32(
                           _mm_unpacklo_epi16(v_zero_i, v_src_12), 16)),
                       v_scale),
            v_shift);
        v_dst1 = _mm_max_ps(_mm_sub_ps(v_zero_f, v_dst1), v_dst1);
        __m128 v_dst2 = _mm_add_ps(
            _mm_mul_ps(_mm_cvtepi32_ps(_mm_srai_epi32(
                           _mm_unpackhi_epi16(v_zero_i, v_src_12), 16)),
                       v_scale),
            v_shift);
        v_dst2 = _mm_max_ps(_mm_sub_ps(v_zero_f, v_dst2), v_dst2);
        __m128 v_dst3 = _mm_add_ps(
            _mm_mul_ps(_mm_cvtepi32_ps(_mm_srai_epi32(
                           _mm_unpacklo_epi16(v_zero_i, v_src_34), 16)),
                       v_scale),
            v_shift);
        v_dst3 = _mm_max_ps(_mm_sub_ps(v_zero_f, v_dst3), v_dst3);
        __m128 v_dst4 = _mm_add_ps(
            _mm_mul_ps(_mm_cvtepi32_ps(_mm_srai_epi32(
                           _mm_unpackhi_epi16(v_zero_i, v_src_34), 16)),
                       v_scale),
            v_shift);
        v_dst4 = _mm_max_ps(_mm_sub_ps(v_zero_f, v_dst4), v_dst4);
  ...
  int operator()(const ushort* src,
    ...
    if (USE_SSE2) {
      ...
      for (; x <= width - 8; x += 8) {
        __m128i v_src = _mm_loadu_si128((const __m128i*)(src + x));
        __m128 v_dst1 = _mm_add_ps(
            _mm_mul_ps(_mm_cvtepi32_ps(_mm_unpacklo_epi16(v_src, v_zero_i)),
                       v_scale),
            v_shift);
        v_dst1 = _mm_max_ps(_mm_sub_ps(v_zero_f, v_dst1), v_dst1);
        __m128 v_dst2 = _mm_add_ps(
            _mm_mul_ps(_mm_cvtepi32_ps(_mm_unpackhi_epi16(v_src, v_zero_i)),
                       v_scale),
            v_shift);
        v_dst2 = _mm_max_ps(_mm_sub_ps(v_zero_f, v_dst2), v_dst2);
...
struct cvtScaleAbs_SIMD<short, uchar, float> {
  int operator()(
      const short* src, uchar* dst, int width, float scale, float shift) const {
    int x = 0;
    ...
    if (USE_SSE2) {
      ...
      for (; x <= width - 8; x += 8) {
        __m128i v_src = _mm_loadu_si128((const __m128i*)(src + x));
        __m128 v_dst1 =
            _mm_add_ps(_mm_mul_ps(_mm_cvtepi32_ps(_mm_srai_epi32(
                                      _mm_unpacklo_epi16(v_src, v_src), 16)),
                                  v_scale),
                       v_shift);
        v_dst1 = _mm_max_ps(_mm_sub_ps(v_zero_f, v_dst1), v_dst1);
        __m128 v_dst2 =
            _mm_add_ps(_mm_mul_ps(_mm_cvtepi32_ps(_mm_srai_epi32(
                                      _mm_unpackhi_epi16(v_src, v_src), 16)),
                                  v_scale),
                       v_shift);
        v_dst2 = _mm_max_ps(_mm_sub_ps(v_zero_f, v_dst2), v_dst2);
...
struct cvtScaleAbs_SIMD<int, uchar, float> {
  int operator()(
      const int* src, uchar* dst, int width, float scale, float shift) const {
    int x = 0;
    ...
    if (USE_SSE2) {
      ...
      for (; x <= width - 8; x += 4) {
        __m128i v_src = _mm_loadu_si128((const __m128i*)(src + x));
        __m128 v_dst1 =
            _mm_add_ps(_mm_mul_ps(_mm_cvtepi32_ps(v_src), v_scale), v_shift);
        v_dst1 = _mm_max_ps(_mm_sub_ps(v_zero_f, v_dst1), v_dst1);

        __m128i v_dst_i = _mm_packus_epi16(
            _mm_packs_epi32(_mm_cvtps_epi32(v_dst1), v_zero_i), v_zero_i);
        _mm_storel_epi64((__m128i*)(dst + x), v_dst_i);
      }
    }
...
struct cvtScaleAbs_SIMD<float, uchar, float> {
  int operator()(
      const float* src, uchar* dst, int width, float scale, float shift) const {
    int x = 0;
    ...
    if (USE_SSE2) {
      ...
      for (; x <= width - 8; x += 4) {
        __m128 v_dst =
            _mm_add_ps(_mm_mul_ps(_mm_loadu_ps(src + x), v_scale), v_shift);
        ...
        _mm_storel_epi64((__m128i*)(dst + x),
                         _mm_packus_epi16(v_dst_i, v_zero_i));
      }
    }
...
struct cvtScaleAbs_SIMD<uchar, uchar, float> {
  int operator()(
    ...
    for (; x <= width - 16; x += 16) {
      ...
      vst1q_u8(dst + x,
               vcombine_u8(vqmovn_u16(v_dsti_0), vqmovn_u16(v_dsti_1)));
    }
...
struct cvtScaleAbs_SIMD<int, uchar, float> {
  int operator()(
    ...
    for (; x <= width - 8; x += 8) {
      float32x4_t v_dst_0 =
          vmulq_n_f32(vcvtq_f32_s32(vld1q_s32(src + x)), scale);
      v_dst_0 = vabsq_f32(vaddq_f32(v_dst_0, v_shift));
      uint16x4_t v_dsti_0 = vqmovn_u32(cv_vrndq_u32_f32(v_dst_0));

      float32x4_t v_dst_1 =
          vmulq_n_f32(vcvtq_f32_s32(vld1q_s32(src + x + 4)), scale);
      v_dst_1 = vabsq_f32(vaddq_f32(v_dst_1, v_shift));
...
template <typename T, typename DT, typename WT>
static void cvtScaleAbs_(const T* src,
                         size_t sstep,
                         DT* dst,
                         size_t dstep,
                         Size size,
...
struct cvtScale_SIMD<uchar, uchar, float> {
  int operator()(
    ...
    for (; x <= width - 8; x += 8) {
      __m128i v_src =
          _mm_unpacklo_epi8(_mm_loadl_epi64((__m128i const*)(src + x)), v_zero);
      ...
      __m128i v_dst =
          _mm_packs_epi32(_mm_cvtps_epi32(v_dst_0), _mm_cvtps_epi32(v_dst_1));
...
template <>
struct cvtScale_SIMD<uchar, char, float> {
  int operator()(
      const uchar* src, char* dst, int width, float scale, float shift) const {
    int x = 0;
...
struct cvtScale_SIMD<uchar, ushort, float> {
  cvtScale_SIMD() {
    haveSSE = checkHardwareSupport(CIL_CPU_SSE4_1);
  }

  int operator()(const uchar* src,
                 ushort* dst,
                 int width,
                 float scale,
                 float shift) const {
    ...
    for (; x <= width - 8; x += 8) {
      ...
      __m128i v_dst =
          _mm_packus_epi32(_mm_cvtps_epi32(v_dst_0), _mm_cvtps_epi32(v_dst_1));
...
struct cvtScale_SIMD<uchar, short, float> {
  int operator()(
      const uchar* src, short* dst, int width, float scale, float shift) const {
    int x = 0;
...
struct cvtScale_SIMD<uchar, int, float> {
  int operator()(
      const uchar* src, int* dst, int width, float scale, float shift) const {
    int x = 0;
...
struct cvtScale_SIMD<uchar, float, float> {
  int operator()(
      const uchar* src, float* dst, int width, float scale, float shift) const {
    int x = 0;
...
struct cvtScale_SIMD<uchar, double, double> {
  int operator()(const uchar* src,
    ...
    for (; x <= width - 8; x += 8) {
      ...
      __m128d v_dst_0 =
          _mm_add_pd(_mm_mul_pd(_mm_cvtepi32_pd(v_src_s32), v_scale), v_shift);
      __m128d v_dst_1 = _mm_add_pd(
          _mm_mul_pd(_mm_cvtepi32_pd(_mm_srli_si128(v_src_s32, 8)), v_scale),
          v_shift);
...
template <>
struct cvtScale_SIMD<char, uchar, float> {
  int operator()(
      const char* src, uchar* dst, int width, float scale, float shift) const {
    int x = 0;
    ...
    for (; x <= width - 8; x += 8) {
      __m128i v_src = _mm_srai_epi16(
          _mm_unpacklo_epi8(v_zero, _mm_loadl_epi64((__m128i const*)(src + x))),
          8);
      __m128 v_src_f = _mm_cvtepi32_ps(
          _mm_srai_epi32(_mm_unpacklo_epi16(v_zero, v_src), 16));
      __m128 v_dst_0 = _mm_add_ps(_mm_mul_ps(v_src_f, v_scale), v_shift);

      v_src_f = _mm_cvtepi32_ps(
...
template <>
struct cvtScale_SIMD<char, char, float> {
  int operator()(
      const char* src, char* dst, int width, float scale, float shift) const {
    int x = 0;
...
template <>
struct cvtScale_SIMD<char, ushort, float> {
  cvtScale_SIMD() {
    haveSSE = checkHardwareSupport(CIL_CPU_SSE4_1);
  }
...
template <>
struct cvtScale_SIMD<char, short, float> {
  int operator()(
      const char* src, short* dst, int width, float scale, float shift) const {
    int x = 0;
...
template <>
struct cvtScale_SIMD<char, int, float> {
  int operator()(
      const char* src, int* dst, int width, float scale, float shift) const {
    int x = 0;
...
template <>
struct cvtScale_SIMD<char, float, float> {
  int operator()(
      const char* src, float* dst, int width, float scale, float shift) const {
    int x = 0;
...
template <>
struct cvtScale_SIMD<char, double, double> {
  int operator()(const char* src,
                 double* dst,
                 int width,
                 double scale,
...
template <>
struct cvtScale_SIMD<ushort, char, float> {
  int operator()(const ushort* src,
                 char* dst,
                 int width,
                 float scale,
...
struct cvtScale_SIMD<ushort, int, float> {
  int operator()(
      const ushort* src, int* dst, int width, float scale, float shift) const {
    int x = 0;
...
template <>
struct cvtScale_SIMD<short, char, float> {
  int operator()(
      const short* src, char* dst, int width, float scale, float shift) const {
    int x = 0;
...
struct cvtScale_SIMD<short, short, float> {
  int operator()(
      const short* src, short* dst, int width, float scale, float shift) const {
    int x = 0;
...
struct cvtScale_SIMD<short, int, float> {
  int operator()(
      const short* src, int* dst, int width, float scale, float shift) const {
    int x = 0;
...
struct cvtScale_SIMD<short, float, float> {
  int operator()(
      const short* src, float* dst, int width, float scale, float shift) const {
    int x = 0;
...
template <>
struct cvtScale_SIMD<int, char, float> {
  int operator()(
      const int* src, char* dst, int width, float scale, float shift) const {
    int x = 0;
...
struct cvtScale_SIMD<int, ushort, float> {
  ...
  int operator()(
      const int* src, ushort* dst, int width, float scale, float shift) const {
    int x = 0;
...
struct cvtScale_SIMD<int, short, float> {
  int operator()(
      const int* src, short* dst, int width, float scale, float shift) const {
    int x = 0;
...
struct cvtScale_SIMD<int, int, double> {
  int operator()(
      const int* src, int* dst, int width, double scale, double shift) const {
    int x = 0;
    ...
    for (; x <= width - 4; x += 4) {
      __m128i v_src = _mm_loadu_si128((__m128i const*)(src + x));
      __m128d v_dst_0 =
          _mm_add_pd(_mm_mul_pd(_mm_cvtepi32_pd(v_src), v_scale), v_shift);
...
struct cvtScale_SIMD<int, float, double> {
  int operator()(
      const int* src, float* dst, int width, double scale, double shift) const {
    int x = 0;
    ...
    for (; x <= width - 4; x += 4) {
      ...
      _mm_storeu_ps(
          dst + x, _mm_movelh_ps(_mm_cvtpd_ps(v_dst_0), _mm_cvtpd_ps(v_dst_1)));
    }
...
template <>
struct cvtScale_SIMD<float, char, float> {
  int operator()(
      const float* src, char* dst, int width, float scale, float shift) const {
    int x = 0;
...
struct cvtScale_SIMD<float, short, float> {
  int operator()(
      const float* src, short* dst, int width, float scale, float shift) const {
    int x = 0;
...
struct cvtScale_SIMD<float, int, float> {
  int operator()(
      const float* src, int* dst, int width, float scale, float shift) const {
    int x = 0;
...
struct cvtScale_SIMD<float, float, float> {
  int operator()(
      const float* src, float* dst, int width, float scale, float shift) const {
    int x = 0;
...
struct cvtScale_SIMD<float, double, double> {
  int operator()(const float* src,
    ...
    for (; x <= width - 4; x += 4) {
      __m128 v_src = _mm_loadu_ps(src + x);
      __m128d v_dst_0 =
          _mm_add_pd(_mm_mul_pd(_mm_cvtps_pd(v_src), v_scale), v_shift);
...
template <>
struct cvtScale_SIMD<double, char, float> {
  int operator()(const double* src,
                 char* dst,
                 int width,
                 float scale,
...
struct cvtScale_SIMD<uchar, uchar, float> {
  int operator()(
    ...
    for (; x <= width - 8; x += 8) {
      ...
      float32x4_t v_dst1 = vaddq_f32(
          vmulq_f32(vcvtq_f32_u32(vmovl_u16(vget_low_u16(v_src))), v_scale),
          v_shift);
      float32x4_t v_dst2 = vaddq_f32(
          vmulq_f32(vcvtq_f32_u32(vmovl_u16(vget_high_u16(v_src))), v_scale),
          v_shift);
...
struct cvtScale_SIMD<uchar, float, float> {
  int operator()(
    ...
    for (; x <= width - 8; x += 8) {
      ...
      vst1q_f32(
          dst + x + 4,
          vaddq_f32(vmulq_f32(vcvtq_f32_u32(vmovl_u16(vget_high_u16(v_src))),
                              v_scale),
                    v_shift));
    }
...
struct cvtScale_SIMD<char, uchar, float> {
  int operator()(
    ...
    for (; x <= width - 8; x += 8) {
      ...
      float32x4_t v_dst1 = vaddq_f32(
          vmulq_f32(vcvtq_f32_s32(vmovl_s16(vget_low_s16(v_src))), v_scale),
          v_shift);
      float32x4_t v_dst2 = vaddq_f32(
          vmulq_f32(vcvtq_f32_s32(vmovl_s16(vget_high_s16(v_src))), v_scale),
          v_shift);
...
struct cvtScale_SIMD<char, float, float> {
  int operator()(
    ...
    for (; x <= width - 8; x += 8) {
      ...
      vst1q_f32(
          dst + x + 4,
          vaddq_f32(vmulq_f32(vcvtq_f32_s32(vmovl_s16(vget_high_s16(v_src))),
                              v_scale),
                    v_shift));
    }
...
struct cvtScale_SIMD<int, uchar, float> {
  int operator()(
    ...
    for (; x <= width - 8; x += 8) {
      float32x4_t v_dst1 = vaddq_f32(
          vmulq_f32(vcvtq_f32_s32(vld1q_s32(src + x)), v_scale), v_shift);
      float32x4_t v_dst2 = vaddq_f32(
          vmulq_f32(vcvtq_f32_s32(vld1q_s32(src + x + 4)), v_scale), v_shift);
...
struct cvtScale_SIMD<float, uchar, float> {
  int operator()(
    ...
    for (; x <= width - 8; x += 8) {
      float32x4_t v_dst1 =
          vaddq_f32(vmulq_f32(vld1q_f32(src + x), v_scale), v_shift);
      float32x4_t v_dst2 =
          vaddq_f32(vmulq_f32(vld1q_f32(src + x + 4), v_scale), v_shift);
...
struct cvtScale_SIMD<float, int, float> {
  int operator()(
    ...
    for (; x <= width - 4; x += 4)
      vst1q_s32(dst + x, cv_vrndq_s32_f32(vaddq_f32(
                             vmulq_f32(vld1q_f32(src + x), v_scale), v_shift)));
...
template <typename T, typename DT, typename WT>
static void cvtScale_(const T* src,
                      size_t sstep,
                      DT* dst,
                      size_t dstep,
                      Size size,
...
template <>
void cvtScale_<short, short, float>(const short* src,
                                    size_t sstep,
                                    short* dst,
                                    size_t dstep,
                                    Size size,
  ...
  for (; size.height--; src += sstep, dst += dstep) {
    ...
    if (USE_SSE2) {
      ...
      for (; x <= size.width - 8; x += 8) {
        ...
        __m128 rf0 =
            _mm_cvtepi32_ps(_mm_srai_epi32(_mm_unpacklo_epi16(r0, r0), 16));
        __m128 rf1 =
            _mm_cvtepi32_ps(_mm_srai_epi32(_mm_unpacklo_epi16(r1, r1), 16));
...
template <>
void cvtScale_<short, int, float>(const short* src,
                                  size_t sstep,
                                  int* dst,
                                  size_t dstep,
                                  Size size,
    ...
    if (USE_SSE2)  //~5X
    {
      ...
      for (; x <= size.width - 8; x += 8) {
        ...
        __m128 rf0 =
            _mm_cvtepi32_ps(_mm_srai_epi32(_mm_unpacklo_epi16(r0, r0), 16));
        __m128 rf1 =
            _mm_cvtepi32_ps(_mm_srai_epi32(_mm_unpackhi_epi16(r0, r0), 16));
        rf0 = _mm_add_ps(_mm_mul_ps(rf0, scale128), shift128);
        rf1 = _mm_add_ps(_mm_mul_ps(rf1, scale128), shift128);
...
template <>
struct Cvt_SIMD<double, char> {
  int operator()(const double* src, char* dst, int width) const {
    int x = 0;
...
struct Cvt_SIMD<double, ushort> {
  bool haveSIMD;
  Cvt_SIMD() {
    haveSIMD = checkHardwareSupport(CIL_CPU_SSE4_1);
  }
  ...
  int operator()(const double* src, ushort* dst, int width) const {
    ...
    for (; x <= width - 8; x += 8) {
      ...
      __m128i v_dst =
          _mm_packus_epi32(_mm_cvtps_epi32(v_src0), _mm_cvtps_epi32(v_src1));
      _mm_storeu_si128((__m128i*)(dst + x), v_dst);
    }
...
template <>
struct Cvt_SIMD<uchar, char> {
  int operator()(const uchar* src, char* dst, int width) const {
    int x = 0;

    for (; x <= width - 8; x += 8)
      vst1_s8(dst + x,
              vqmovn_s16(vreinterpretq_s16_u16(vmovl_u8(vld1_u8(src + x)))));
...
struct Cvt_SIMD<uchar, int> {
  int operator()(const uchar* src, int* dst, int width) const {
    ...
    for (; x <= width - 8; x += 8) {
      ...
      vst1q_s32(dst + x + 4,
                vreinterpretq_s32_u32(vmovl_u16(vget_high_u16(v_src))));
    }
...
template <>
struct Cvt_SIMD<char, uchar> {
  int operator()(const char* src, uchar* dst, int width) const {
    int x = 0;
...
template <>
struct Cvt_SIMD<char, short> {
  int operator()(const char* src, short* dst, int width) const {
    int x = 0;
...
template <>
struct Cvt_SIMD<char, ushort> {
  int operator()(const char* src, ushort* dst, int width) const {
    int x = 0;

    for (; x <= width - 8; x += 8) {
      int16x8_t v_src = vmovl_s8(vld1_s8(src + x));
      vst1q_u16(dst + x,
                vcombine_u16(vqmovun_s32(vmovl_s16(vget_low_s16(v_src))),
...
template <>
struct Cvt_SIMD<char, int> {
  int operator()(const char* src, int* dst, int width) const {
    int x = 0;
...
template <>
struct Cvt_SIMD<char, float> {
  int operator()(const char* src, float* dst, int width) const {
    int x = 0;
...
template <>
struct Cvt_SIMD<ushort, char> {
  int operator()(const ushort* src, char* dst, int width) const {
    int x = 0;

    for (; x <= width - 16; x += 16) {
      uint16x8_t v_src1 = vld1q_u16(src + x), v_src2 = vld1q_u16(src + x + 8);
      int32x4_t v_dst10 =
          vreinterpretq_s32_u32(vmovl_u16(vget_low_u16(v_src1)));
      int32x4_t v_dst11 =
          vreinterpretq_s32_u32(vmovl_u16(vget_high_u16(v_src1)));
      int32x4_t v_dst20 =
          vreinterpretq_s32_u32(vmovl_u16(vget_low_u16(v_src2)));
      int32x4_t v_dst21 =
          vreinterpretq_s32_u32(vmovl_u16(vget_high_u16(v_src2)));

      vst1q_s8(dst + x,
               vcombine_s8(vqmovn_s16(vcombine_s16(vqmovn_s32(v_dst10),
                                                   vqmovn_s32(v_dst11))),
                           vqmovn_s16(vcombine_s16(vqmovn_s32(v_dst20),
                                                   vqmovn_s32(v_dst21)))));
    }
...
template <>
struct Cvt_SIMD<short, char> {
  int operator()(const short* src, char* dst, int width) const {
    int x = 0;
...
struct Cvt_SIMD<int, uchar> {
  int operator()(const int* src, uchar* dst, int width) const {
    ...
    for (; x <= width - 16; x += 16) {
      ...
      int32x4_t v_src3 = vld1q_s32(src + x + 8),
                v_src4 = vld1q_s32(src + x + 12);
      uint8x8_t v_dst1 =
          vqmovn_u16(vcombine_u16(vqmovun_s32(v_src1), vqmovun_s32(v_src2)));
      uint8x8_t v_dst2 =
          vqmovn_u16(vcombine_u16(vqmovun_s32(v_src3), vqmovun_s32(v_src4)));
...
template <>
struct Cvt_SIMD<int, char> {
  int operator()(const int* src, char* dst, int width) const {
    int x = 0;
    ...
    for (; x <= width - 16; x += 16) {
      ...
      int32x4_t v_src3 = vld1q_s32(src + x + 8),
                v_src4 = vld1q_s32(src + x + 12);
      int8x8_t v_dst1 =
          vqmovn_s16(vcombine_s16(vqmovn_s32(v_src1), vqmovn_s32(v_src2)));
      int8x8_t v_dst2 =
          vqmovn_s16(vcombine_s16(vqmovn_s32(v_src3), vqmovn_s32(v_src4)));
...
struct Cvt_SIMD<int, ushort> {
  int operator()(const int* src, ushort* dst, int width) const {
    ...
    for (; x <= width - 8; x += 8) {
      int32x4_t v_src1 = vld1q_s32(src + x), v_src2 = vld1q_s32(src + x + 4);
      vst1q_u16(dst + x,
                vcombine_u16(vqmovun_s32(v_src1), vqmovun_s32(v_src2)));
    }
...
struct Cvt_SIMD<float, uchar> {
  int operator()(const float* src, uchar* dst, int width) const {
    ...
    for (; x <= width - 16; x += 16) {
      ...
      uint8x8_t v_dst1 =
          vqmovn_u16(vcombine_u16(vqmovn_u32(v_src1), vqmovn_u32(v_src2)));
      uint8x8_t v_dst2 =
          vqmovn_u16(vcombine_u16(vqmovn_u32(v_src3), vqmovn_u32(v_src4)));
      vst1q_u8(dst + x, vcombine_u8(v_dst1, v_dst2));
    }
...
template <>
struct Cvt_SIMD<float, char> {
  int operator()(const float* src, char* dst, int width) const {
    int x = 0;
...
static short convertFp16SW(float fp32) {
  ...
  if (0x477ff000 <= absolute) {
    ...
    int threshold =
        ((0x400000 >> bitShift) |
         (((significand & (0x800000 >> bitShift)) >> (126 - a.fmt.exponent)) ^
          1));
...
template <typename T, typename DT>
static void cvtScaleHalf_(
    const T* src, size_t sstep, DT* dst, size_t dstep, Size size);

template <>
void cvtScaleHalf_<float, short>(
    const float* src, size_t sstep, short* dst, size_t dstep, Size size) {
  sstep /= sizeof(src[0]);
  dstep /= sizeof(dst[0]);

  if (checkHardwareSupport(CIL_CPU_FP16)) {
    for (; size.height--; src += sstep, dst += dstep) {
      int x = 0;
...
template <>
void cvtScaleHalf_<short, float>(
    const short* src, size_t sstep, float* dst, size_t dstep, Size size) {
  sstep /= sizeof(src[0]);
  dstep /= sizeof(dst[0]);
...
template <typename T, typename DT>
static void cvt_(const T* src, size_t sstep, DT* dst, size_t dstep, Size size) {
  sstep /= sizeof(src[0]);
  dstep /= sizeof(dst[0]);
  Cvt_SIMD<T, DT> vop;
...
template <typename T>
static void cpy_(const T* src, size_t sstep, T* dst, size_t dstep, Size size) {
  sstep /= sizeof(src[0]);
  dstep /= sizeof(dst[0]);
...
#define DEF_CVT_SCALE_ABS_FUNC(suffix, tfunc, stype, dtype, wtype)          \
  static void cvtScaleAbs##suffix(const stype* src, size_t sstep,           \
                                  const uchar*, size_t, dtype* dst,         \
...
#define DEF_CVT_SCALE_FP16_FUNC(suffix, stype, dtype)                  \
  static void cvtScaleHalf##suffix(const stype* src, size_t sstep,     \
                                   const uchar*, size_t, dtype* dst,   \
                                   size_t dstep, Size size, double*) { \
    cvtScaleHalf_<stype, dtype>(src, sstep, dst, dstep, size);         \
  }

#define DEF_CVT_SCALE_FUNC(suffix, stype, dtype, wtype)                        \
  static void cvtScale##suffix(const stype* src, size_t sstep, const uchar*,   \
                               size_t, dtype* dst, size_t dstep, Size size,    \
                               double* scale) {                                \
...
#define DEF_CVT_FUNC_F(suffix, stype, dtype, ippFavor)                  \
  static void cvt##suffix(const stype* src, size_t sstep, const uchar*, \
                          size_t, dtype* dst, size_t dstep, Size size,  \
                          double*) {                                    \
...
#define DEF_CPY_FUNC(suffix, stype)                                     \
  static void cvt##suffix(const stype* src, size_t sstep, const uchar*, \
                          size_t, stype* dst, size_t dstep, Size size,  \
                          double*) {                                    \
    cpy_(src, sstep, dst, dstep, size);                                 \
  }

DEF_CVT_SCALE_ABS_FUNC(8u, cvtScaleAbs_, uchar, uchar, float)
DEF_CVT_SCALE_ABS_FUNC(8s8u, cvtScaleAbs_, char, uchar, float)
...
DEF_CVT_SCALE_FUNC(8s8u, char, uchar, float)
...
DEF_CVT_SCALE_FUNC(8u8s, uchar, char, float)
DEF_CVT_SCALE_FUNC(8s, char, char, float)
DEF_CVT_SCALE_FUNC(16u8s, ushort, char, float)
DEF_CVT_SCALE_FUNC(16s8s, short, char, float)
DEF_CVT_SCALE_FUNC(32s8s, int, char, float)
DEF_CVT_SCALE_FUNC(32f8s, float, char, float)
DEF_CVT_SCALE_FUNC(64f8s, double, char, float)

DEF_CVT_SCALE_FUNC(8u16u, uchar, ushort, float)
DEF_CVT_SCALE_FUNC(8s16u, char, ushort, float)
...
DEF_CVT_SCALE_FUNC(8s16s, char, short, float)
...
DEF_CVT_SCALE_FUNC(8s32s, char, int, float)
...
DEF_CVT_SCALE_FUNC(8s32f, char, float, float)
...
DEF_CVT_SCALE_FUNC(8s64f, char, double, double)
...
DEF_CVT_FUNC_F(8s8u, char, uchar, 8s8u_C1Rs)
...
DEF_CVT_FUNC_F2(8u8s, uchar, char, 8u8s_C1RSfs)
DEF_CVT_FUNC_F2(16u8s, ushort, char, 16u8s_C1RSfs)
DEF_CVT_FUNC_F2(16s8s, short, char, 16s8s_C1RSfs)
DEF_CVT_FUNC_F(32s8s, int, char, 32s8s_C1R)
DEF_CVT_FUNC_F2(32f8s, float, char, 32f8s_C1RSfs)
DEF_CVT_FUNC(64f8s, double, char)

DEF_CVT_FUNC_F(8u16u, uchar, ushort, 8u16u_C1R)
DEF_CVT_FUNC_F(8s16u, char, ushort, 8s16u_C1Rs)
...
DEF_CVT_FUNC_F(8s16s, char, short, 8s16s_C1R)
...
DEF_CVT_FUNC_F(8s32s, char, int, 8s32s_C1R)
...
DEF_CVT_FUNC_F(8s32f, char, float, 8s32f_C1R)
...
BinaryFunc getConvertFuncFp16(int ddepth) {
  static BinaryFunc cvtTab[] = {
      0, 0,
      0, (BinaryFunc)(cvtScaleHalf32f16f),
      0, (BinaryFunc)(cvtScaleHalf16f32f),
      0, 0,
  };
  return cvtTab[CIL_MAT_DEPTH(ddepth)];
}

BinaryFunc getConvertFunc(int sdepth, int ddepth) {
  static BinaryFunc cvtTab[][8] = {
      {(BinaryFunc)(cvt8u), (BinaryFunc)GET_OPTIMIZED(cvt8s8u),
       (BinaryFunc)GET_OPTIMIZED(cvt16u8u), (BinaryFunc)GET_OPTIMIZED(cvt16s8u),
       (BinaryFunc)GET_OPTIMIZED(cvt32s8u), (BinaryFunc)GET_OPTIMIZED(cvt32f8u),
       (BinaryFunc)GET_OPTIMIZED(cvt64f8u), 0},
      {(BinaryFunc)GET_OPTIMIZED(cvt8u8s), (BinaryFunc)cvt8u,
       (BinaryFunc)GET_OPTIMIZED(cvt16u8s), (BinaryFunc)GET_OPTIMIZED(cvt16s8s),
       (BinaryFunc)GET_OPTIMIZED(cvt32s8s), (BinaryFunc)GET_OPTIMIZED(cvt32f8s),
       (BinaryFunc)GET_OPTIMIZED(cvt64f8s), 0},
      {(BinaryFunc)GET_OPTIMIZED(cvt8u16u), (BinaryFunc)GET_OPTIMIZED(cvt8s16u),
       (BinaryFunc)cvt16u, (BinaryFunc)GET_OPTIMIZED(cvt16s16u),
       (BinaryFunc)GET_OPTIMIZED(cvt32s16u),
       (BinaryFunc)GET_OPTIMIZED(cvt32f16u),
       (BinaryFunc)GET_OPTIMIZED(cvt64f16u), 0},
      {(BinaryFunc)GET_OPTIMIZED(cvt8u16s), (BinaryFunc)GET_OPTIMIZED(cvt8s16s),
       (BinaryFunc)GET_OPTIMIZED(cvt16u16s), (BinaryFunc)cvt16u,
       (BinaryFunc)GET_OPTIMIZED(cvt32s16s),
       (BinaryFunc)GET_OPTIMIZED(cvt32f16s),
       (BinaryFunc)GET_OPTIMIZED(cvt64f16s), 0},
      {(BinaryFunc)GET_OPTIMIZED(cvt8u32s), (BinaryFunc)GET_OPTIMIZED(cvt8s32s),
       (BinaryFunc)GET_OPTIMIZED(cvt16u32s),
       (BinaryFunc)GET_OPTIMIZED(cvt16s32s), (BinaryFunc)cvt32s,
       (BinaryFunc)GET_OPTIMIZED(cvt32f32s),
       (BinaryFunc)GET_OPTIMIZED(cvt64f32s), 0},
      {(BinaryFunc)GET_OPTIMIZED(cvt8u32f), (BinaryFunc)GET_OPTIMIZED(cvt8s32f),
       (BinaryFunc)GET_OPTIMIZED(cvt16u32f),
       (BinaryFunc)GET_OPTIMIZED(cvt16s32f),
       (BinaryFunc)GET_OPTIMIZED(cvt32s32f), (BinaryFunc)cvt32s,
       (BinaryFunc)GET_OPTIMIZED(cvt64f32f), 0},
      {(BinaryFunc)GET_OPTIMIZED(cvt8u64f), (BinaryFunc)GET_OPTIMIZED(cvt8s64f),
       (BinaryFunc)GET_OPTIMIZED(cvt16u64f),
       (BinaryFunc)GET_OPTIMIZED(cvt16s64f),
       (BinaryFunc)GET_OPTIMIZED(cvt32s64f),
       (BinaryFunc)GET_OPTIMIZED(cvt32f64f), (BinaryFunc)(cvt64s), 0},
      {0, 0, 0, 0, 0, 0, 0, 0}};

  return cvtTab[CIL_MAT_DEPTH(ddepth)][CIL_MAT_DEPTH(sdepth)];
}

static BinaryFunc getConvertScaleFunc(int sdepth, int ddepth) {
  static BinaryFunc cvtScaleTab[][8] = {
      {(BinaryFunc)GET_OPTIMIZED(cvtScale8u),
       (BinaryFunc)GET_OPTIMIZED(cvtScale8s8u),
       (BinaryFunc)GET_OPTIMIZED(cvtScale16u8u),
       (BinaryFunc)GET_OPTIMIZED(cvtScale16s8u),
       (BinaryFunc)GET_OPTIMIZED(cvtScale32s8u),
       (BinaryFunc)GET_OPTIMIZED(cvtScale32f8u), (BinaryFunc)cvtScale64f8u, 0},
      {(BinaryFunc)GET_OPTIMIZED(cvtScale8u8s),
       (BinaryFunc)GET_OPTIMIZED(cvtScale8s),
       (BinaryFunc)GET_OPTIMIZED(cvtScale16u8s),
       (BinaryFunc)GET_OPTIMIZED(cvtScale16s8s),
       (BinaryFunc)GET_OPTIMIZED(cvtScale32s8s),
       (BinaryFunc)GET_OPTIMIZED(cvtScale32f8s), (BinaryFunc)cvtScale64f8s, 0},
      {(BinaryFunc)GET_OPTIMIZED(cvtScale8u16u),
       (BinaryFunc)GET_OPTIMIZED(cvtScale8s16u),
       (BinaryFunc)GET_OPTIMIZED(cvtScale16u),
       (BinaryFunc)GET_OPTIMIZED(cvtScale16s16u),
       (BinaryFunc)GET_OPTIMIZED(cvtScale32s16u),
       (BinaryFunc)GET_OPTIMIZED(cvtScale32f16u), (BinaryFunc)cvtScale64f16u,
       0},
      {(BinaryFunc)GET_OPTIMIZED(cvtScale8u16s),
       (BinaryFunc)GET_OPTIMIZED(cvtScale8s16s),
       (BinaryFunc)GET_OPTIMIZED(cvtScale16u16s),
       (BinaryFunc)GET_OPTIMIZED(cvtScale16s),
       (BinaryFunc)GET_OPTIMIZED(cvtScale32s16s),
       (BinaryFunc)GET_OPTIMIZED(cvtScale32f16s), (BinaryFunc)cvtScale64f16s,
       0},
      {(BinaryFunc)GET_OPTIMIZED(cvtScale8u32s),
       (BinaryFunc)GET_OPTIMIZED(cvtScale8s32s),
       (BinaryFunc)GET_OPTIMIZED(cvtScale16u32s),
       (BinaryFunc)GET_OPTIMIZED(cvtScale16s32s),
       (BinaryFunc)GET_OPTIMIZED(cvtScale32s),
       (BinaryFunc)GET_OPTIMIZED(cvtScale32f32s), (BinaryFunc)cvtScale64f32s,
       0},
      {(BinaryFunc)GET_OPTIMIZED(cvtScale8u32f),
       (BinaryFunc)GET_OPTIMIZED(cvtScale8s32f),
       (BinaryFunc)GET_OPTIMIZED(cvtScale16u32f),
       (BinaryFunc)GET_OPTIMIZED(cvtScale16s32f),
       (BinaryFunc)GET_OPTIMIZED(cvtScale32s32f),
       (BinaryFunc)GET_OPTIMIZED(cvtScale32f), (BinaryFunc)cvtScale64f32f, 0},
  ...
  return cvtScaleTab[CIL_MAT_DEPTH(ddepth)][CIL_MAT_DEPTH(sdepth)];
}

void convertScaleAbs(InputArray _src,
                     OutputArray _dst,
                     double alpha,
                     double beta) {
  Mat src = _src.getMat();
  ...
  _dst.create(src.dims, src.size, CIL_8UC(cn));
  Mat dst = _dst.getMat();
...
void convertFp16(InputArray _src, OutputArray _dst) {
  ...
  case CIL_16S:
    ...
    break;
  default:
    CIL_ERROR("Unsupported input depth");
    return;
  }

  int type = CIL_MAKETYPE(ddepth, src.channels());
...
void Mat::convertTo(OutputArray _dst,
                    int _type,
                    double alpha,
                    double beta) const {
  ...
  if (_type < 0)
    ...
    _type = CIL_MAKETYPE(CIL_MAT_DEPTH(_type), channels());

  int sdepth = depth(), ddepth = CIL_MAT_DEPTH(_type);
  ...
  BinaryFunc func = noScale ? getConvertFunc(sdepth, ddepth)
                            : getConvertScaleFunc(sdepth, ddepth);
  double scale[] = {alpha, beta};
  int cn = channels();
  CIL_ASSERT(func != 0);
...
void convertAndUnrollScalar(const Mat& sc,
                            ...
                            uchar* scbuf,
                            size_t blocksize) {
  int scn = (int)sc.total(), cn = CIL_MAT_CN(buftype);
  size_t esz = CIL_ELEM_SIZE(buftype);
  getConvertFunc(sc.depth(), buftype)(sc.ptr(), 1, 0, 1, scbuf, 1,
                                      Size(std::min(cn, scn), 1), 0);
  // unroll the scalar
  if (scn < cn) {
    CIL_ASSERT(scn == 1);
    size_t esz1 = CIL_ELEM_SIZE1(buftype);
