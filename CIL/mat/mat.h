...
#include "CIL/range.h"
#include "CIL/rect.h"
#include "CIL/scalar.h"
#include "CIL/mat/allocator.h"
#include "CIL/mat/io_array.h"
#include "CIL/mat/iterator.h"
#include "CIL/mat/size.h"
#include "CIL/mat/step.h"
#include "CIL/mat/umat_data.h"
#include "CIL/mat/vec.h"
...
class CIL_EXPORTS Mat {
 public:
  ...
  expression or can be allocated with Mat::create. In the former case, the old
  ...
  @param type Array type. Use CIL_8UC1, ..., CIL_64FC4 to create 1-4 channel
  matrices, or CIL_8UC(n), ..., CIL_64FC(n) to create multi-channel (up to
  CIL_CN_MAX channels) matrices.
   */
  ...
  last step is always set to the element size). If not specified, the matrix
  ...
  is assumed to be continuous.
  */
  Mat(const std::vector<int>& sizes,
      int type,
      void* data,
      const size_t* steps = 0);
  ...
  */
  ...
  is NULL, and you should not deallocate the data until the matrix is not
  destructed.
  */
  ...
  */
  ...
  Remember that each of these operations is O(1), that is, no data is copied.
  Thus, the above assignment is not true if you may have expected the j-th row
  to be copied to the i-th row. To achieve that, you should either turn this
  simple assignment into an expression or use the Mat::copyTo method:
  @code
      Mat A;
      ...
  ...
  */
  ...
  - `d>0` is a diagonal from the lower half. For example, d=1 means the diagonal
  is set immediately below the main one.
  ...
  The mask has to be of type CIL_8U and can have 1 or multiple channels.
  */
  ...
  */
  ...
  single-column or
  single-row vectors, the top-to-bottom left-to-right scan ordering is used to
  treat them as 1D
  vectors. The vectors must have the same size and type. If the matrices have
  ...
  -# Allocate the new, associated with the data, reference counter and set it to
  1.

  Such a scheme makes the memory management robust and efficient at the same
  time and helps avoid
  extra typing for you. This means that usually there is no need to explicitly
  ...
  */
  ...
  If the matrix header
  points to an external data set (see Mat::Mat ), the reference counter is NULL,
  and the method has no
  effect in this case. Normally, to avoid memory leaks, the method should not be
  called explicitly. It
  is called implicitly by the matrix assignment operator. The reference counter
  increment is an atomic
  operation on the platforms that support it. Thus, it is safe to operate on the
  same matrices
  ...
  automatically called in the destructor, or by any other method that changes
  the data pointer, it is
  usually not needed. The reference counter decrement and check for 0 is an
  atomic operation on the
  platforms that support it. Thus, it is safe to operate on the same matrices
  asynchronously in
  different threads.
   */
  void release();
  ...
  to store sz rows,
  nothing happens. If the matrix is reallocated, the first Mat::rows rows are
  ...
  the first
  min(Mat::rows, sz) rows are preserved. The methods emulate the corresponding
  methods of the STL
  vector class.
  ...
  the corresponding
  method of the STL vector class. When elem is Mat , its type and the number of
  columns must be the
  ...
  */
  ...
  Mat::rowRange,
  Mat::colRange, and others, the resultant submatrix points just to the part of
  the original big
  matrix. However, each submatrix contains information (represented by datastart
  and dataend
  ...
  the submatrix position within the parent matrix and then shift the position
  somehow. Typically, it
  can be required for filtering operations when pixels outside of the ROI should
  be taken into
  account. When all the method parameters are positive, the ROI needs to grow in
  ...
  The matrix is shifted
  by 2 elements to the left and 2 elements up, which brings in all the necessary
  pixels for the
  filtering with the 5x5 kernel.

  adjustROI forces the adjusted ROI to be inside of the parent matrix that is
  boundaries of the
  adjusted ROI are constrained by boundaries of the parent matrix. For example,
  if the submatrix A is
  located in the first row of a parent matrix and you called A.adjustROI(2, 2,
  2, 2) then A will not
  ...
  */
  ...
  always continuous.
  Matrices created with Mat::create are always continuous. But if you extract a
  part of the matrix
  ...
  (such as arithmetic and logical operations, math functions, alpha blending,
  color space
  transformations, and others) do not depend on the image geometry. Thus, if all
  the input and output
  arrays are continuous, the functions can process them as very long single-row
  vectors. The example
  ...
  @code
      ...
      void alphaBlendRGBA(const Mat& src1, const Mat& src2, Mat& dst)
      {
          ...
          CIL_ASSERT( src1.type() == src2.type() &&
                     src1.type() == CIL_MAKETYPE(DataType<T>::depth, 4) &&
                     src1.size() == src2.size());
          Size size = src1.size();
  ...
  ptr2[j+1]*beta);
                  dptr[j+2] = saturate_cast<T>(ptr1[j+2]*alpha +
  ptr2[j+2]*beta);
                  dptr[j+3] = saturate_cast<T>((1 -
  ...
  element-operation by
  10-20 percents, especially if the image is rather small and the operation is
  quite simple.
  ...
  channels. For example, if the matrix type is CIL_16SC3 , the method returns
  sizeof(short) or 2.
   */
  ...
  with the CvMat type
  system, like CIL_16SC3 or 16-bit signed 3-channel array, and so on.
   */
  int type() const;
  ...
  For example, for a 16-bit signed element array, the method returns CIL_16S .
  A complete list of
  matrix types contains the following values:
  -   CIL_8U - 8-bit unsigned integers ( 0..255 )
  -   CIL_8S - 8-bit signed integers ( -128..127 )
  -   CIL_16U - 16-bit unsigned integers ( 0..65535 )
  -   CIL_16S - 16-bit signed integers ( -32768..32767 )
  -   CIL_32S - 32-bit signed integers ( -2147483648..2147483647 )
  -   CIL_32F - 32-bit floating-point numbers ( -FLT_MAX..FLT_MAX, INF, NAN )
  -   CIL_64F - 64-bit floating-point numbers ( -DBL_MAX..DBL_MAX, INF, NAN )
   */
  int depth() const;
  ...
  */
  ...
  of single-row or
  single-column 2-dimensional arrays. That is, if, for example, A is a 1 x N
  ...
  at random. It depends
  on the image from which you are trying to retrieve the data. The table below
  gives a better insight in this:
   - If matrix is of type `CIL_8U` then use `Mat.at<uchar>(y,x)`.
   - If matrix is of type `CIL_8S` then use `Mat.at<char>(y,x)`.
   - If matrix is of type `CIL_16U` then use `Mat.at<ushort>(y,x)`.
   - If matrix is of type `CIL_16S` then use `Mat.at<short>(y,x)`.
   - If matrix is of type `CIL_32S`  then use `Mat.at<int>(y,x)`.
   - If matrix is of type `CIL_32F`  then use `Mat.at<float>(y,x)`.
   - If matrix is of type `CIL_64F` then use `Mat.at<double>(y,x)`.
  ...
  */
  ...
  matrix iterators is very
  similar to the use of bi-directional STL iterators. In the example below, the
  alpha blending
  ...
  position.
      ...
      Mat_<Pixel> image = Mat::zeros(3, sizes, CIL_8UC3);
  ...
  enum {
    ...
    AUTO_STEP = 0,
    CONTINUOUS_FLAG = CIL_MAT_CONT_FLAG,
    SUBMATRIX_FLAG = CIL_SUBMAT_FLAG
  };
