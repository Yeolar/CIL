...
NAryMatIterator::NAryMatIterator(const Mat** _arrays,
                                 Mat* _planes,
                                 int _narrays)
    : arrays(0),
      planes(0),
...
void NAryMatIterator::init(const Mat** _arrays,
                           Mat* _planes,
                           uchar** _ptrs,
                           int _narrays) {
  CIL_ASSERT(_arrays && (_ptrs || _planes));
  ...
  if (narrays < 0) {
    for (i = 0; _arrays[i] != 0; i++)
      ;
    narrays = i;
    CIL_ASSERT(narrays <= 1000);
  }

  iterdepth = 0;

  for (i = 0; i < narrays; i++) {
    CIL_ASSERT(arrays[i] != 0);
    ...
    if (i0 < 0) {
      ...
      CIL_ASSERT(A.size == arrays[i0]->size);

    if (!A.isContinuous()) {
      CIL_ASSERT(A.step[d - 1] == A.elemSize());
      for (j = d - 1; j > d1; j--)
