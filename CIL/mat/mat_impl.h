...
inline Mat::Mat(int _rows, int _cols, int _type, void* _data, size_t _step)
    : flags(MAGIC_VAL + (_type & TYPE_MASK)),
      ...
      u(0),
      size(&rows) {
  CIL_ASSERT(total() == 0 || data != NULL);

  size_t esz = CIL_ELEM_SIZE(_type), esz1 = CIL_ELEM_SIZE1(_type);
...
inline Mat::Mat(const std::vector<_Tp>& vec, bool copyData)
    : flags(MAGIC_VAL | DataType<_Tp>::type | CIL_MAT_CONT_FLAG),
      dims(2),
      rows((int)vec.size()),
      cols(1),
      data(0),
...
inline int Mat::depth() const {
  return CIL_MAT_DEPTH(flags);
}

inline int Mat::channels() const {
  return CIL_MAT_CN(flags);
}
...
inline uchar* Mat::ptr(int i0, int i1) {
  ...
  assert((unsigned)i1 < (unsigned)size.p[1]);
...
inline uchar* Mat::ptr(int i0, int i1, int i2) {
  ...
  assert((unsigned)i0 < (unsigned)size.p[0]);
  assert((unsigned)i1 < (unsigned)size.p[1]);
  assert((unsigned)i2 < (unsigned)size.p[2]);
...
inline _Tp& Mat::at(int i0, int i1) {
  ...
  assert(data);
  assert((unsigned)i0 < (unsigned)size.p[0]);
  assert((unsigned)(i1 * DataType<_Tp>::channels) <
                  (unsigned)(size.p[1] * channels()));
  assert(CIL_ELEM_SIZE1(DataType<_Tp>::depth) == elemSize1());
...
inline _Tp& Mat::at(Point pt) {
  assert(dims <= 2);
  assert(data);
  assert((unsigned)pt.y < (unsigned)size.p[0]);
  assert((unsigned)(pt.x * DataType<_Tp>::channels) <
                  (unsigned)(size.p[1] * channels()));
  assert(CIL_ELEM_SIZE1(DataType<_Tp>::depth) == elemSize1());
...
inline _Tp& Mat::at(int i0) {
  assert(dims <= 2);
  assert(data);
  assert((unsigned)i0 < (unsigned)(size.p[0] * size.p[1]));
  assert(elemSize() == CIL_ELEM_SIZE(DataType<_Tp>::type));
...
inline Mat::operator Vec<_Tp, n>() const {
  CIL_ASSERT(data && dims <= 2 && (rows == 1 || cols == 1) &&
...
inline Mat::operator Matx<_Tp, m, n>() const {
  CIL_ASSERT(data && dims <= 2 && rows == m && cols == n && channels() == 1);
...
inline void Mat::push_back(const _Tp& elem) {
  ...
  CIL_ASSERT(DataType<_Tp>::type == type() && cols == 1
