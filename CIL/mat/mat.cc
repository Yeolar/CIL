...
#include "CIL/error.h"
#include "CIL/xadd.h"
#include "CIL/transform/rotate.h"

namespace cil {

static inline void setSize(Mat& m,
                           int _dims,
                           const int* _sz,
                           const size_t* _steps,
                           bool autoSteps = false) {
  CIL_ASSERT(0 <= _dims && _dims <= CIL_MAX_DIM);
  if (m.dims != _dims) {
    ...
    if (_dims > 2) {
      m.step.p = (size_t*)fastMalloc(_dims * sizeof(m.step.p[0]) +
  ...
  size_t esz = CIL_ELEM_SIZE(m.flags), esz1 = CIL_ELEM_SIZE1(m.flags),
         total = esz;
  for (int i = _dims - 1; i >= 0; i--) {
    ...
    if (_steps) {
      if (_steps[i] % esz1 != 0) {
        CIL_ERROR("Step must be a multiple of esz1");
      }
      ...
      if ((uint64_t)total1 != (size_t)total1)
        CIL_ERROR("The total matrix size does not fit to \"size_t\" type");
      total = (size_t)total1;
    }
  }
...
void Mat::create(int d, const int* _sizes, int _type) {
  int i;
  CIL_ASSERT(0 <= d && d <= CIL_MAX_DIM && _sizes);
  _type = CIL_MAT_TYPE(_type);
  ...
  int _sizes_backup[CIL_MAX_DIM];  // #5991
  ...
  if (d == 0)
    return;
  flags = (_type & CIL_MAT_TYPE_MASK) | MAGIC_VAL;
  ...
  if (total() > 0) {
...
#ifdef CIL_USE_NUMPY_ALLOCATOR_BY_DEFAULT
    ...
    CIL_ASSERT(step[dims - 1] == (size_t)CIL_ELEM_SIZE(flags));
  }
  addref();
  finalizeHdr(*this);
}
...
Mat::Mat(const Mat& m, const Range& _rowRange, const Range& _colRange)
  ...
  *this = m;
  if (_rowRange != Range::all() && _rowRange != Range(0, rows)) {
    CIL_ASSERT(0 <= _rowRange.start && _rowRange.start <= _rowRange.end &&
  ...
  if (_colRange != Range::all() && _colRange != Range(0, cols)) {
    CIL_ASSERT(0 <= _colRange.start && _colRange.start <= _colRange.end &&
...
Mat::Mat(const Mat& m, const Rect& roi)
  ...
  size_t esz = CIL_ELEM_SIZE(flags);
  data += roi.x * esz;
  CIL_ASSERT(0 <= roi.x && 0 <= roi.width && roi.x + roi.width <= m.cols &&
               0 <= roi.y && 0 <= roi.height && roi.y + roi.height <= m.rows);
  if (u)
    CIL_XADD(&u->refcount, 1);
...
Mat::Mat(
    int _dims, const int* _sizes, int _type, void* _data, const size_t* _steps)
    : flags(MAGIC_VAL),
      dims(0),
      rows(0),
      cols(0),
      ...
      u(0),
      size(&rows) {
  flags |= CIL_MAT_TYPE(_type);
  datastart = data = (uchar*)_data;
  setSize(*this, _dims, _sizes, _steps, true);
  finalizeHdr(*this);
}

Mat::Mat(const std::vector<int>& _sizes,
         int _type,
         void* _data,
         const size_t* _steps)
    : flags(MAGIC_VAL),
...
void Mat::pop_back(size_t nelems) {
  CIL_ASSERT(nelems <= (size_t)size.p[0]);
...
void Mat::push_back(const Mat& elems) {
  ...
  size.p[0] = r;
  if (!eq)
    CIL_ERROR("Pushed vector length is not equal to matrix row length");
  if (type() != elems.type())
    CIL_ERROR("Pushed vector type is not the same as matrix type");
...
void Mat::locateROI(Size& wholeSize, Point& ofs) const {
  ...
  if (delta1 == 0)
    ...
    assert(data == datastart + ofs.y * step[0] + ofs.x * esz);
  }
...
Mat Mat::reshape(int new_cn, int new_rows) const {
  ...
  if (dims > 2 && new_rows == 0 && new_cn != 0 &&
    ...
    hdr.flags =
        (hdr.flags & ~CIL_MAT_CN_MASK) | ((new_cn - 1) << CIL_CN_SHIFT);
    hdr.step[dims - 1] = CIL_ELEM_SIZE(hdr.flags);
  ...
  if (new_rows != 0 && new_rows != rows) {
    ...
    if (!isContinuous())
      CIL_ERROR(
          "The matrix is not continuous, thus its number of rows can not be "
          "changed");

    if ((unsigned)new_rows > (unsigned)total_size)
      CIL_ERROR("Bad new number of rows");

    total_width = total_size / new_rows;

    if (total_width * new_rows != total_size)
      CIL_ERROR(
          "The total number of matrix elements is not divisible by the new "
          "number of rows");
  ...
  hdr.flags =
      (hdr.flags & ~CIL_MAT_CN_MASK) | ((new_cn - 1) << CIL_CN_SHIFT);
  hdr.step[1] = CIL_ELEM_SIZE(hdr.flags);
  return hdr;
}

Mat Mat::diag(const Mat& d) {
  CIL_ASSERT(d.cols == 1 || d.rows == 1);
...
int Mat::checkVector(int _elemChannels,
                     int _depth,
...
Mat Mat::reshape(int _cn, int _newndims, const int* _newsz) const {
  ...
  if (isContinuous()) {
    CIL_ASSERT(_cn >= 0 && _newndims > 0 && _newndims <= CIL_MAX_DIM &&
                 _newsz);

    if (_cn == 0)
      _cn = this->channels();
    else
      CIL_ASSERT(_cn <= CIL_CN_MAX);
    ...
    for (int i = 0; i < _newndims; i++) {
      ...
      if (_newsz[i] > 0)
        ...
        CIL_ERROR(
            "Copy dimension (which has zero size) is not present in source "
            "matrix");
    ...
    Mat hdr = *this;
    hdr.flags =
        (hdr.flags & ~CIL_MAT_CN_MASK) | ((_cn - 1) << CIL_CN_SHIFT);
  ...
  CIL_ERROR(
      "Reshaping of n-dimensional non-continuous matrices is not supported "
      "yet");
  // TBD
  return Mat();
}
