...
Mat _InputArray::getMat_(int i) const {
  ...
  if (k == STD_VECTOR_VECTOR) {
    int t = type(i);
    const std::vector<std::vector<uchar>>& vv =
        *(const std::vector<std::vector<uchar>>*)obj;
    CIL_ASSERT(0 <= i && i < (int)vv.size());
  ...
  if (k == STD_VECTOR_MAT) {
    ...
    CIL_ASSERT(0 <= i && i < (int)v.size());

    return v[i];
  }

  CIL_ERROR("Unknown/unsupported array type");
  return Mat();
}
...
void _InputArray::getMatVector(std::vector<Mat>& mv) const {
  ...
  if (k == MAT) {
    ...
    for (int i = 0; i < n; i++)
      mv[i] = m.dims == 2 ? Mat(1, m.cols, m.type(), (void*)m.ptr(i))
                          : Mat(m.dims - 1, &m.size[1], m.type(),
                                (void*)m.ptr(i), &m.step[1]);
    return;
  }

  if (k == MATX) {
    size_t n = sz.height, esz = CIL_ELEM_SIZE(flags);
    mv.resize(n);

    for (size_t i = 0; i < n; i++)
      mv[i] = Mat(1, sz.width, CIL_MAT_TYPE(flags),
  ...
  if (k == STD_VECTOR) {
    ...
    size_t n = v.size(), esz = CIL_ELEM_SIZE(flags);
    int t = CIL_MAT_DEPTH(flags), cn = CIL_MAT_CN(flags);
    mv.resize(n);
...
Size _InputArray::size(int i) const {
  ...
  if (k == STD_VECTOR) {
    ...
    return szb == szi ? Size((int)szb, 1)
                      : Size((int)(szb / CIL_ELEM_SIZE(flags)), 1);
  }
  ...
  if (k == STD_VECTOR_VECTOR) {
    ...
    CIL_ASSERT(i < (int)vv.size());
    const std::vector<std::vector<int>>& ivv =
...
int _InputArray::depth(int i) const {
  return CIL_MAT_DEPTH(type(i));
}

int _InputArray::channels(int i) const {
  return CIL_MAT_CN(type(i));
}
...
bool _InputArray::isContinuous(int i) const {
  ...
  if (k == MATX || k == STD_VECTOR || k == NONE || k == STD_VECTOR_VECTOR ||
      k == STD_BOOL_VECTOR)
    return true;

  if (k == STD_VECTOR_MAT) {
    const std::vector<Mat>& vv = *(const std::vector<Mat>*)obj;
    CIL_ASSERT((size_t)i < vv.size());
...
size_t _InputArray::offset(int i) const {
  ...
  if (k == EXPR || k == MATX || k == STD_VECTOR || k == NONE ||
...
void _OutputArray::create(Size _sz,
                          int mtype,
                          int i,
  ...
  if (k == MAT && i < 0 && !allowTransposed && fixedDepthMask == 0) {
    CIL_ASSERT(!fixedSize() || ((Mat*)obj)->size.operator()() == _sz);
    CIL_ASSERT(!fixedType() || ((Mat*)obj)->type() == mtype);
...
void _OutputArray::create(int _rows,
                          int _cols,
                          int mtype,
                          int i,
                          bool allowTransposed,
  ...
  int k = kind();
  if (k == MAT && i < 0 && !allowTransposed && fixedDepthMask == 0) {
    CIL_ASSERT(!fixedSize() ||
                 ((Mat*)obj)->size.operator()() == Size(_cols, _rows));
    CIL_ASSERT(!fixedType() || ((Mat*)obj)->type() == mtype);
...
void _OutputArray::create(int d,
                          const int* sizes,
                          int mtype,
                          int i,
                          bool allowTransposed,
  ...
  if (k == MAT) {
    ...
    if (allowTransposed) {
      if (!m.isContinuous()) {
        CIL_ASSERT(!fixedType() && !fixedSize());
        m.release();
      }
    ...
    if (fixedType()) {
      if (CIL_MAT_CN(mtype) == m.channels() &&
          ((1 << CIL_MAT_TYPE(flags)) & fixedDepthMask) != 0)
        mtype = m.type();
      else
        CIL_ASSERT(CIL_MAT_TYPE(mtype) == m.type());
    }
    if (fixedSize()) {
      CIL_ASSERT(m.dims == d);
      for (int j = 0; j < d; ++j)
        CIL_ASSERT(m.size[j] == sizes[j]);
    }
  ...
  if (k == MATX) {
    CIL_ASSERT(i < 0);
    int type0 = CIL_MAT_TYPE(flags);
    CIL_ASSERT(mtype == type0 || (CIL_MAT_CN(mtype) == 1 &&
                                    ((1 << type0) & fixedDepthMask) != 0));
    CIL_ASSERT(d == 2 && ((sizes[0] == sz.height && sizes[1] == sz.width) ||
  ...
  if (k == STD_VECTOR || k == STD_VECTOR_VECTOR) {
    CIL_ASSERT(d == 2 &&
                 (sizes[0] == 1 || sizes[1] == 1 || sizes[0] * sizes[1] == 0));
    size_t len = sizes[0] * sizes[1] > 0 ? sizes[0] + sizes[1] - 1 : 0;
    std::vector<uchar>* v = (std::vector<uchar>*)obj;

    if (k == STD_VECTOR_VECTOR) {
      std::vector<std::vector<uchar>>& vv =
          *(std::vector<std::vector<uchar>>*)obj;
      if (i < 0) {
        CIL_ASSERT(!fixedSize() || len == vv.size());
        vv.resize(len);
        return;
      }
      CIL_ASSERT(i < (int)vv.size());
    ...
    int type0 = CIL_MAT_TYPE(flags);
    CIL_ASSERT(mtype == type0 ||
                 (CIL_MAT_CN(mtype) == CIL_MAT_CN(type0) &&
                  ((1 << type0) & fixedDepthMask) != 0));

    int esz = CIL_ELEM_SIZE(type0);
    CIL_ASSERT(!fixedSize() || len == ((std::vector<uchar>*)v)->size() / esz);
    switch (esz) {
    case 1:
  ...
  if (k == NONE) {
    CIL_ERROR("create() called for the missing output array");
    return;
  }
  ...
  if (k == STD_VECTOR_MAT) {
    ...
    if (i < 0) {
      CIL_ASSERT(d == 2 && (sizes[0] == 1 || sizes[1] == 1 ||
                              sizes[0] * sizes[1] == 0));
      size_t len = sizes[0] * sizes[1] > 0 ? sizes[0] + sizes[1] - 1 : 0,
             len0 = v.size();

      CIL_ASSERT(!fixedSize() || len == len0);
      v.resize(len);
      if (fixedType()) {
        int _type = CIL_MAT_TYPE(flags);
        for (size_t j = len0; j < len; j++) {
          if (v[j].type() == _type)
            continue;
          CIL_ASSERT(v[j].empty());
          v[j].flags = (v[j].flags & ~CIL_MAT_TYPE_MASK) | _type;
        }
      }
      return;
    }

    CIL_ASSERT(i < (int)v.size());
    Mat& m = v[i];
...
void _OutputArray::createSameSize(const _InputArray& arr, int mtype) const {
  int arrsz[CIL_MAX_DIM], d = arr.sizend(arrsz);
...
void _OutputArray::release() const {
  ...
  if (k == STD_VECTOR) {
    create(Size(), CIL_MAT_TYPE(flags));
    return;
  }
...
void _OutputArray::setTo(const _InputArray& arr,
                         const _InputArray& mask) const {
  int k = kind();
