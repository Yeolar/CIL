...
TlsAbstraction::TlsAbstraction() {
  tlsKey = TlsAlloc();
  CIL_ASSERT(tlsKey != TLS_OUT_OF_INDEXES);
}
...
void TlsAbstraction::SetData(void* pData) {
  CIL_ASSERT(TlsSetValue(tlsKey, pData) == TRUE);
}
...
TlsAbstraction::TlsAbstraction() {
  CIL_ASSERT(pthread_key_create(&tlsKey, NULL) == 0);
}

TlsAbstraction::~TlsAbstraction() {
  CIL_ASSERT(pthread_key_delete(tlsKey) == 0);
}
...
void TlsAbstraction::SetData(void* pData) {
  CIL_ASSERT(pthread_setspecific(tlsKey, pData) == 0);
}
...
class TlsStorage {
 public:
  ...
  void releaseSlot(size_t slotIdx, std::vector<void*>& dataVec) {
    // std::lock_guard<std::mutex> guard(mtxGlobalAccess);
    CIL_ASSERT(tlsSlots.size() > slotIdx);
  ...
  void setData(size_t slotIdx, void* pData) {
    CIL_ASSERT(tlsSlots.size() > slotIdx && pData != NULL);
 ...
 private:
  ...
  // Array for all allocated data. Thread data pointers are placed here to allow
  // data cleanup
...
static TlsStorage& getTlsStorage() {
  return *Singleton<TlsStorage>::getInstance();
}
...
TLSDataContainer::~TLSDataContainer() {
  CIL_ASSERT(key_ == -1);  // Key must be released in child object
}
