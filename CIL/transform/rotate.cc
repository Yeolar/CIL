...
static void flipHoriz(const uchar* src,
                      size_t sstep,
                      uchar* dst,
                      size_t dstep,
                      Size size,
...
static void flipVert(const uchar* src0,
                     size_t sstep,
                     uchar* dst0,
                     size_t dstep,
                     Size size,
  ...
  for (int y = 0; y < (size.height + 1) / 2;
       y++, src0 += sstep, src1 -= sstep, dst0 += dstep, dst1 -= dstep) {
    int i = 0;
...
static void transpose_(
    const uchar* src, size_t sstep, uchar* dst, size_t dstep, Size sz) {
...
void transpose(InputArray _src, OutputArray _dst) {
  int type = _src.type(), esz = CIL_ELEM_SIZE(type);
  CIL_ASSERT(_src.dims() <= 2 && esz <= 32);
  ...
  // handle the case of single-column/single-row matrices, stored in STL
  // vectors.
  if (src.rows != dst.cols || src.cols != dst.rows) {
    CIL_ASSERT(src.size() == dst.size() && (src.cols == 1 || src.rows == 1));
    src.copyTo(dst);
    return;
  }
