...
{
    CIL_ASSERT((dst_width * dst_height > 0) || (inv_scale_x > 0 && inv_scale_y > 0));
    ...
    int depth = CIL_MAT_DEPTH(src_type), cn = CIL_MAT_CN(src_type);
    ...
    if (interpolation == INTER_CUBIC)
        ...
        CIL_ERROR("Unknown interpolation method");
    ksize2 = ksize / 2;
...
void resize(InputArray _src, OutputArray _dst, Size dsize, double inv_scale_x, double inv_scale_y, int interpolation)
{
    ...
    CIL_ASSERT(ssize.width > 0 && ssize.height > 0);
    CIL_ASSERT(dsize.area() > 0 || (inv_scale_x > 0 && inv_scale_y > 0));
    if (dsize.area() == 0)
    {
