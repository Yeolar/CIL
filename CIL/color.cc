...
void convert_BGR2Gray_8u_C3C1R(const uchar* rgb,
                               int rgb_step,
                               uchar* gray,
                               int gray_step,
                               Size size,
  ...
  for (; size.height--; gray += gray_step) {
    for (i = 0; i < size.width; i++, rgb += 3) {
      int t = descale(rgb[swap_rb] * cB + rgb[1] * cG + rgb[swap_rb ^ 2] * cR,
                      SCALE);
      gray[i] = (uchar)t;
    }
...
void convert_BGRA2Gray_16u_CnC1R(const ushort* rgb,
                                 int rgb_step,
                                 ushort* gray,
                                 int gray_step,
                                 Size size,
...
void convert_BGRA2Gray_8u_C4C1R(const uchar* rgba,
                                int rgba_step,
                                uchar* gray,
                                int gray_step,
                                Size size,
  ...
  for (; size.height--; gray += gray_step) {
    for (i = 0; i < size.width; i++, rgba += 4) {
      int t = descale(
          rgba[swap_rb] * cB + rgba[1] * cG + rgba[swap_rb ^ 2] * cR, SCALE);
      gray[i] = (uchar)t;
    }
...
void convert_RGBA2RGB_8u_C4C3R(const uchar* rgba,
                               int rgba_step,
                               uchar* rgb,
                               int rgb_step,
                               Size size,
...
void convert_BGRA2BGR_16u_C4C3R(const ushort* bgra,
                                int bgra_step,
                                ushort* bgr,
                                int bgr_step,
                                Size size,
...
void convert_BGR5552Gray_8u_C2C1R(const uchar* bgr555,
                                  int bgr555_step,
                                  uchar* gray,
                                  int gray_step,
                                  Size size) {
...
void convert_BGR5652Gray_8u_C2C1R(const uchar* bgr565,
                                  int bgr565_step,
                                  uchar* gray,
                                  int gray_step,
                                  Size size) {
...
void convert_CMYK2Gray_8u_C4C1R(
    const uchar* cmyk, int cmyk_step, uchar* gray, int gray_step, Size size) {
  int i;
  for (; size.height--;) {
...
void CvtPaletteToGray(const PaletteEntry* palette,
  ...
  int i;
  for (i = 0; i < entries; i++) {
    convert_BGR2Gray_8u_C3C1R((uchar*)(palette + i), 0, grayPalette + i, 0,
                              Size(1, 1));
  }
}
...
uchar* FillColorRow8(uchar* data,
                     uchar* indices,
                     int len,
...
uchar* FillColorRow4(uchar* data,
                     uchar* indices,
                     int len,
                     PaletteEntry* palette) {
  uchar* end = data + len * 3;
...
uchar* FillColorRow1(uchar* data,
                     uchar* indices,
                     int len,
                     PaletteEntry* palette) {
  uchar* end = data + len * 3;
